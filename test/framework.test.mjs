...
test("emits identical static-path route entries once", async t => {
  const fixture = new URL("./fixtures/route-entry-dedup", import.meta.url)
  t.after(async () => {
    await rm(new URL("./fixtures/route-entry-dedup/.kudzu", import.meta.url), { recursive: true, force: true })
    await rm(new URL("./fixtures/route-entry-dedup/dist", import.meta.url), { recursive: true, force: true })
  })
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  const output = new URL("./fixtures/route-entry-dedup/dist/", import.meta.url)
  const html = await Promise.all(["oak", "pine", "vine"].map(id => readFile(new URL(`items/${id}/index.html`, output), "utf8")))
  const entry = kind => html.map(source => source.match(new RegExp(`src="([^"]+/assets/${kind}/[^"]+\\.js)"`))?.[1])
  assert.equal(new Set(entry("native")).size, 1)
  assert.equal(new Set(entry("effects")).size, 1)
...
test("inlines critical read-only query form carry", async t => {
  const fixture = new URL("./fixtures/query-form-carry", import.meta.url)
  t.after(async () => {
    await rm(new URL("./fixtures/query-form-carry/.kudzu", import.meta.url), { recursive: true, force: true })
    await rm(new URL("./fixtures/query-form-carry/dist", import.meta.url), { recursive: true, force: true })
  })
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  const html = await readFile(new URL("./fixtures/query-form-carry/dist/index.html", import.meta.url), "utf8")
  const staticHtml = await readFile(new URL("./fixtures/query-form-carry/dist/static/index.html", import.meta.url), "utf8")
  assert.equal((html.match(/data-k-query-carry/g) ?? []).length, 3)
...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.30/index.html", import.meta.url), "utf8")
  ...
  assert.ok(docs.includes(`@kudzujs/core@^${version} typescript`))
  assert.ok(docs.includes(`Current ${version} release evidence`))
  assert.match(docs, /R19 AI-authoring confirmation did not reproduce.*1\.0\.0.*blocked/s)
  assert.match(docs, /138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.ok(examples.includes(`Release ${version}`) && examples.includes(`v${version}`))
  assert.match(examples, /KUDZU EXAMPLE CATALOG/)
  ...
  assert.match(html, /Release website coverage/)
  ...
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.30"/)
  assert.match(release, /Only used property handlers.*Bounded element discovery.*Shared owners retain capabilities.*Measured output, unchanged goals/s)
...
test("emits configured global styles in every document head", async t => {
  ...
  for (const route of ["", "about/"]) {
    ...
    assert.equal(html.match(/href="\/guide\/assets\/base\.css"/g)?.length, 1)
...
test("emits independent shared-layout navigation groups", async t => {
  ...
  assert.match(alpha, /href="\/app\/assets\/styles\/shell-a\.css" data-k-route-style/)
  assert.match(alpha, /href="\/app\/assets\/styles\/alpha\.css" data-k-route-style/)
  assert.doesNotMatch(alpha, /styles\/(?:item|shell-b|beta|gamma)\.css/)
  assert.match(beta, /href="\/app\/assets\/styles\/shell-b\.css" data-k-route-style/)
  assert.match(beta, /href="\/app\/assets\/styles\/beta\.css" data-k-route-style/)
  assert.doesNotMatch(beta, /styles\/(?:shell-a|alpha|item|gamma)\.css/)
  assert.doesNotMatch(outside, /data-k-route-style|assets\/styles\//)
...
test("patches reactive attributes and form properties without a VDOM", async t => {
  ...
  assert.doesNotMatch(bindingRuntime, /data-k-if|data-k-true|data-k-false/, "attribute/text bindings must not ship unused conditional DOM ownership")
...
test("compiles conditional DOM branches with nested behavior", async t => {
  ...
  assert.equal(plan.conditions.length, 18)
...
test("compiles synchronous rendered collection selectors and React positional keys", async t => {
  ...
  assert.match(html, /data-search-count[^>]*>[\s\S]*3[\s\S]* articles/)
  assert.match(html, /data-search-results[\s\S]*"keys":\["a","b","c"\]/)
...
test("owns setter callbacks and object refs across three component boundaries", async t => {
  ...
  const staticHtml = await readFile(new URL("./fixtures/callback-ref-ownership/dist/static/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /id="profile-draft"[^>]*>.*Pine.*<\/span>/)
  assert.match(html, /role="tooltip"[^>]*>Current age help<\/span>/)
  assert.equal(html.match(/role="tooltip"/g)?.length, 2)
  assert.doesNotMatch(staticHtml, /<script/)
  const emitted = (await Promise.all((await readdir(new URL("./fixtures/callback-ref-ownership/dist/", import.meta.url), { recursive: true })).filter(file => file.endsWith(".js")).map(file => readFile(new URL(file, new URL("./fixtures/callback-ref-ownership/dist/", import.meta.url)), "utf8")))).join("\n")
  assert.doesNotMatch(emitted, /function (?:ImportedControls|ImportedControlRow|ImportedButton|ImportedSearchField|ImportedSearch)\b|["']react["']/)
  const signalNames = slots => slots.map(slot => sourceResult.moduleIR.signals[slot].debugName)
  const specializationProp = (file, prop) => sourceResult.componentAnalysis.specializations.find(entry => entry.source?.file === file).props.find(entry => entry.name === prop)
  assert.deepEqual(signalNames(specializationProp("src/ImportedControls.tsx", "onPress").signals), ["count"])
  assert.deepEqual(signalNames(specializationProp("src/ImportedSearchField.tsx", "onValueChange").signals), ["query"])
  const chrome = [process.env.CHROME_BIN, "/usr/bin/google-chrome", "/usr/bin/chromium", "/usr/bin/chromium-browser"].find(path => path && existsSync(path))
...
test("fans one setter callback out to multiple direct child handlers", async t => {
  const fixture = new URL("./fixtures/setter-callback-fanout", import.meta.url)
  t.after(async () => {
    await rm(new URL("./fixtures/setter-callback-fanout/.kudzu", import.meta.url), { recursive: true, force: true })
    await rm(new URL("./fixtures/setter-callback-fanout/dist", import.meta.url), { recursive: true, force: true })
  })
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  const html = await readFile(new URL("./fixtures/setter-callback-fanout/dist/index.html", import.meta.url), "utf8")
  assert.match(html, /id="first"[^>]*data-k-on-click='\[\["set","s0","first"\]\]'/)
  assert.match(html, /id="second"[^>]*data-k-on-click='\[\["set","s0","second"\]\]'/)
})
...
test("initializes setter-child state from a direct plain-object state prop", async t => {
  ...
  t.after(async () => {
    await rm(new URL("./fixtures/setter-child-object-initializer-invalid/.kudzu", import.meta.url), { recursive: true, force: true })
    await rm(new URL("./fixtures/setter-child-object-initializer-invalid/dist", import.meta.url), { recursive: true, force: true })
  })
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  const html = await readFile(new URL("./fixtures/setter-child-object-initializer-invalid/dist/index.html", import.meta.url), "utf8")
  const plan = JSON.parse(await readFile(new URL("./fixtures/setter-child-object-initializer-invalid/.kudzu/kudzu-plan.json", import.meta.url), "utf8"))
...
test("rejects setter callbacks across a fourth component boundary", () => {
  const fixture = new URL("./fixtures/setter-third-boundary-invalid", import.meta.url)
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.notEqual(result.status, 0)
  assert.match(`${result.stdout}\n${result.stderr}`, /src\/pages\/index\.tsx:\d+:\d+ Setter callbacks cannot cross more than three component boundaries/)
})
...
test("migrates aliased and member React hooks from a Vite-shaped app", async t => {
  ...
  assert.doesNotMatch(staticHtml, /href="\/app\/assets\/app\.css"/)
  assert.match(staticHtml, /href="\/app\/assets\/download\.css">Download CSS/)
  assert.doesNotMatch(staticHtml, /rel="stylesheet" href="\/app\/assets\/download\.css"/)
  assert.equal(await readFile(new URL("./fixtures/react-vite-app/dist/assets/download.css", import.meta.url), "utf8"), "main { color: teal; }\n")
...
test("compiles a Zustand-shaped store into shared layout state", async t => {
  ...
  assert.match(store, /__kCreateSharedState\("store\.ts#useCart", "quantities", \{\}, \["add", "remove"\], "Zustand"\)/)
  assert.doesNotMatch(emitted, /(?:from\s*|import\s*\()["'](?:react|zustand)["']/)
  for (const source of [shellSource, productSource, cartSource]) assert.deepEqual(source.moduleIR.sharedStates, [{ slot: 0, identity: "store.ts#useCart", field: "quantities", initialValue: {} }])
  assert.deepEqual(productSource.moduleIR.sharedActions, [{ slot: 0, state: 0, name: "add" }])
  assert.deepEqual(cartSource.moduleIR.sharedActions, [{ slot: 0, state: 0, name: "remove" }])
...
test("compiles actions returned through a relative Context hook", async t => {
  ...
  assert.match(handlers, /\.set\("notes"/)
...
test("rejects Context actions whose state is not exposed", () => {
  const fixture = new URL("./fixtures/context-actions-hidden-invalid", import.meta.url)
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.notEqual(result.status, 0)
  assert.match(`${result.stdout}\n${result.stderr}`, /src\/context\.tsx:\d+:\d+ Context action "increment" requires exposed state field "count"/)
})

test("rejects Context setters whose state is not exposed", () => {
  const fixture = new URL("./fixtures/context-actions-setter-only-invalid", import.meta.url)
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.notEqual(result.status, 0)
  assert.match(`${result.stdout}\n${result.stderr}`, /src\/context\.tsx:\d+:\d+ Context Provider setter "setCount" requires exposed state "count"/)
})
...
test("aliases hidden Context action state around consumer bindings", () => {
...
test("bundles package imports used directly by owned effects", async t => {
  const fixture = new URL("./fixtures/effect-package", import.meta.url)
  t.after(async () => {
    await rm(new URL("./fixtures/effect-package/.kudzu", import.meta.url), { recursive: true, force: true })
    await rm(new URL("./fixtures/effect-package/dist", import.meta.url), { recursive: true, force: true })
  })
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  const component = await readFile(new URL("./fixtures/effect-package/.kudzu/pages/index.mjs", import.meta.url), "utf8")
  const handler = await readFile(new URL("./fixtures/effect-package/dist/assets/handlers/pages/index.js", import.meta.url), "utf8")
  const html = await readFile(new URL("./fixtures/effect-package/dist/index.html", import.meta.url), "utf8")
  const staticHtml = await readFile(new URL("./fixtures/effect-package/dist/static/index.html", import.meta.url), "utf8")
  assert.doesNotMatch(component, /typescript|\bts\b/)
  assert.match(handler, /setup:/)
  assert.match(handler, /cleanup:/)
  assert.match(handler, /5\.9\./)
...
test("loads a CodeMirror capability only when its effect owner requests it", async t => {
  const fixture = new URL("./fixtures/lazy-codemirror", import.meta.url)
  t.after(async () => {
    await rm(new URL("./fixtures/lazy-codemirror/.kudzu", import.meta.url), { recursive: true, force: true })
    await rm(new URL("./fixtures/lazy-codemirror/dist", import.meta.url), { recursive: true, force: true })
  })
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`)
  const component = await readFile(new URL("./fixtures/lazy-codemirror/.kudzu/pages/index.mjs", import.meta.url), "utf8")
  const artifacts = JSON.parse(await readFile(new URL("./fixtures/lazy-codemirror/.kudzu/kudzu-artifacts.json", import.meta.url), "utf8"))
  const route = artifacts.routes.find(route => route.route === "/")
  const handler = await readFile(new URL(`./fixtures/lazy-codemirror/dist${route.handlers.entries[0]}`, import.meta.url), "utf8")
  const html = await readFile(new URL("./fixtures/lazy-codemirror/dist/index.html", import.meta.url), "utf8")
  const staticHtml = await readFile(new URL("./fixtures/lazy-codemirror/dist/static/index.html", import.meta.url), "utf8")
  assert.doesNotMatch(component, /@codemirror|EditorView/)
  assert.match(handler, /import\("\.\.\/chunks\//)
  assert.equal(route.handlers.lazyChunks.length > 0, true)
  for (const chunk of route.handlers.lazyChunks) assert.doesNotMatch(html, new RegExp(chunk.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")))
...
test("rejects package imports hidden behind effect helpers", () => {
  const fixture = new URL("./fixtures/effect-package-helper-invalid", import.meta.url)
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.notEqual(result.status, 0)
  assert.match(`${result.stdout}\n${result.stderr}`, /Package import "ts" may only be referenced directly inside JSX event handlers or owned effect setup\/cleanup callbacks/)
})
...
test("rejects lazy package imports without an activation-state guard", () => {
  const fixture = new URL("./fixtures/lazy-package-invalid", import.meta.url)
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.notEqual(result.status, 0)
...
try {
  ...
  await wait()
  if (document.querySelector("#profile-draft").textContent !== "Cedar" || document.querySelector("#profile").textContent !== "Pine") throw new Error("object-draft-independence")
  document.querySelector("#profile-save").click()
  await wait()
  if (document.querySelector("#profile").textContent !== "Cedar") throw new Error("object-draft-commit")
...
http.createServer((request, response) => {
  ...
  response.setHeader("content-type", file.endsWith(".js") ? "text/javascript" : file.endsWith(".css") ? "text/css" : "text/html")
  const send = () => fs.createReadStream(file).on("error", () => { response.statusCode = 404; response.end() }).pipe(response)
  if (file.endsWith("/styles/item.css")) setTimeout(send, 150)
  else send()
}).listen(port, "127.0.0.1")
`
  const server = spawn(process.execPath, ["-e", serverSource, output.pathname, String(port)], { stdio: "ignore" })
...
try {
  ...
  if (document.querySelector("[data-remaining]").textContent !== "2 remaining") throw new Error("scalar-branch-initial")
  ...
  if (document.querySelector("[data-remaining]").textContent !== "1 remaining") throw new Error("scalar-branch-update")
  ...
  if (document.querySelector("[data-remaining]").textContent !== "Done") throw new Error("scalar-branch-replace")
  document.body.dataset.browserTest = "pass"
} catch (error) {
  document.body.dataset.browserTest = "fail-" + error.message
}
...
try {
  ...
  click("search")
  await wait()
  const initialSearchResults = rows("[data-search-results]")
  if (rows("[data-search]").map(node => node.dataset.id).join(",") !== "c" || document.querySelector("[data-search-count]").textContent !== "1 articles" || rows("[data-search-results]").map(node => node.dataset.id).join(",") !== "c") throw new Error("search")
  click("search-none")
  await wait()
  if (document.querySelector("[data-search-count]").textContent !== "0 articles" || !document.querySelector("[data-search-empty]") || document.querySelector("[data-search-results]")) throw new Error("search-empty")
  click("search-clear")
  await wait()
  if (rows("[data-search]").map(node => node.dataset.id).join(",") !== "a,b,c" || document.querySelector("[data-search-count]").textContent !== "3 articles" || rows("[data-search-results]").map(node => node.dataset.id).join(",") !== "a,b,c" || rows("[data-search-results]")[2] === initialSearchResults[0]) throw new Error("search-clear")
...
async function runLazyCodeMirrorBrowserTest(fixture, chrome) {
  await runResourceBrowserScenarios(fixture, chrome, {
    script: `
const waitFor = async (test, label) => {
  for (let index = 0; index < 150; index++) {
...
try {
  ...
  await waitFor(() => document.querySelector(".cm-content")?.textContent === "Lazy editor" && document.body.dataset.lazyEditorMounts === "1" && !document.querySelector('[role="alert"]'), "lazy-mount")
  if (chunkRequests() !== 1) throw new Error("lazy-request")
  document.querySelector("[data-toggle-lazy-editor]").click()
  await waitFor(() => !document.querySelector(".cm-editor") && document.body.dataset.lazyEditorDisposals === "1", "owner-cleanup")
  document.querySelector("[data-toggle-lazy-editor]").click()
  await waitFor(() => document.querySelector(".cm-content")?.textContent === "Lazy editor" && document.body.dataset.lazyEditorMounts === "2", "cached-remount")
  if (chunkRequests() !== 1) throw new Error("duplicate-load")
  window.dispatchEvent(new PageTransitionEvent("pagehide", { persisted: false }))
  await waitFor(() => document.body.dataset.lazyEditorDisposals === "2", "document-cleanup")
  document.body.dataset.lazyCodeMirrorTest = "pass"
} catch (error) {
  document.body.dataset.lazyCodeMirrorTest = "fail-" + error.message
}
`,
...
try {
  ...
  if (reduced) {
    ...
    await waitFor(() => targets.every(target => getComputedStyle(target).opacity === "1" && getComputedStyle(target).visibility === "visible"), "animated-presentation")
