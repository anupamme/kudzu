...
if (process.env.MODE === "retry" && calls === 1) {
...
`, { mode: 0o755 })
  ...
  for (const mode of ["retry", "unauthorized", "mismatch"]) {
    const log = join(root, `${mode}.jsonl`)
    const result = spawnSync("bash", ["-e", "-c", script], { cwd: root, env: { ...process.env, PATH: `${join(root, "bin")}:${process.env.PATH}`, MODE: mode, CALL_LOG: log }, encoding: "utf8", timeout: 30000 })
    const calls = (await readFile(log, "utf8")).trim().split("\n").map(JSON.parse)
    assert.equal(result.status, mode === "retry" ? 0 : 1, result.stderr)
    assert.equal(calls.length, mode === "retry" ? 3 : 12)
    if (mode === "retry") {
